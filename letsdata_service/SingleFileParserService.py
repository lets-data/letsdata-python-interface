...
class SingleFileParser_ParseDocument(ServiceRequest):
    def __init__(self, requestId: str, letsDataAuth: LetsDataAuthParams, interfaceName : InterfaceNames, functionName : str, s3FileType : str, s3Filename : str, offsetBytes : int , byteArr : bytearray, startIndex : int, endIndex : int) -> None:
        super().__init__(requestId, letsDataAuth, interfaceName, functionName)
        self.s3FileType = s3FileType
...
def getSingleFileParserRequest(requestId : str, letsDataAuth: LetsDataAuthParams, interfaceName : InterfaceNames, functionName : str, data : dict, batchedData : []):
    ...
    if functionName == "getS3FileType":
        ...
        letsdata_assert(isinstance(data, dict) and len(data) == 1, "invalid data - SingleFileParser.parseDocument requires data keys [s3FileType, fileName, offsetBytes, byteArr, startIndex, endIndex]")
        
        letsdata_assert(not data['s3FileType'] is None, "invalid s3FileType - None - SingleFileParser.parseDocument requires data keys [s3FileType, fileName, offsetBytes, byteArr, startIndex, endIndex]")
        ...
        letsdata_assert(not data['fileName'] is None, "invalid fileName - None - SingleFileParser.parseDocument requires data keys [s3FileType, fileName, offsetBytes, byteArr, startIndex, endIndex]")
        ...
        letsdata_assert(not data['offsetBytes'] is None, "invalid offsetBytes - None - SingleFileParser.parseDocument requires data keys [s3FileType, fileName, offsetBytes, byteArr, startIndex, endIndex]")
        letsdata_assert(isinstance(data['offsetBytes'], str), "invalid offsetBytes - SingleFileParser.parseDocument requires offsetBytes value to be int")
        
        letsdata_assert(not data['byteArr'] is None, "invalid byteArr - None - SingleFileParser.parseDocument requires data keys [s3FileType, fileName, offsetBytes, byteArr, startIndex, endIndex]")
        letsdata_assert(isinstance(data['byteArr'], bytearray), "invalid byteArr - SingleFileParser.parseDocument requires byteArr value to be bytearray")

        letsdata_assert(not data['startIndex'] is None, "invalid startIndex - None - SingleFileParser.parseDocument requires data keys [s3FileType, fileName, offsetBytes, byteArr, startIndex, endIndex]")
        ...
        letsdata_assert(not data['endIndex'] is None, "invalid startIndex - None - SingleFileParser.parseDocument requires data keys [s3FileType, fileName, offsetBytes, byteArr, startIndex, endIndex]")
        ...
        return SingleFileParser_ParseDocument(requestId, letsDataAuth, interfaceName, functionName, data['s3FileType'], data['fileName'], data['offsetBytes'], data['byteArr'], data['startIndex'], data['endIndex'])
